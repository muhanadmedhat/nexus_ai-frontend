...
const NAV_ITEMS: Record<UserRole, NavItem[]> = {
  customer: [
    { label: "Dashboard", href: "/dashboard/customer", icon: <LayoutDashboard size={20} /> },
  ...
  admin: [
    ...
    { label: "Agent Jobs", href: "/dashboard/admin/agent-jobs", icon: <ListChecks size={20} /> },
    { label: "Reviews", href: "/dashboard/admin/reviews", icon: <Star size={20} /> },
        ...
        <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-4">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={clsx(isActive ? "text-on-primary-container" : "text-outline")}>
                  {item.icon}
                </span>
                {item.label}
              </Link>
        ...
        <div className="mt-auto border-t border-outline-variant/30 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="truncate text-xs capitalize text-on-surface-variant">{role}</p>
            </div>
          </div>
        </div>
      </aside>
